...
public static class Day03
{
    public static IEnumerable<object> Solve(List<string> lines)
    {
        ...
        var parts = numbers.Where(n => n.Positions.SelectMany(p => p.EightAdjacent()).Any(p =>
        ...
        var asteriskParts = asteriskPositions.Select(p => p.EightAdjacent().SelectMany(p => parts.Where(n => n.Positions.Contains(p))).Distinct());
